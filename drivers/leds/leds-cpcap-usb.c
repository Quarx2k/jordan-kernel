				...
				enum led_brightness value)
{
	...
	if (value != LED_OFF) {
		cpcap_status = cpcap_regacc_write(cpusb_data->cpcap,
...
static int ld_cpcap_usbled_probe(struct platform_device *pdev)
{
	...
	ret = cpcap_regacc_read(info->cpcap, CPCAP_REG_CRM, &value);
	if (ret < 0) {
...
static int cpcap_usbled_unregister(void)
{
	...
	ret = cpcap_device_unregister(&cpcap_usbled_device);
...
static int ld_cpcap_usbled_init(void)
{
	int ret;
	...
	ret = cpcap_device_register(&cpcap_usbled_device);
	if (ret < 0) {
...
static void ld_cpcap_usbled_exit(void)
{
