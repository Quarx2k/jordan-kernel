...
static DEFINE_SEMAPHORE(modem_pm_driver_lock);
...
static long modem_pm_driver_ioctl( struct file *file,
				 unsigned int cmd,
				 unsigned long arg);
...
static const struct file_operations modem_pm_driver_fops = {
	.owner =    THIS_MODULE,
	.unlocked_ioctl =    modem_pm_driver_ioctl,
...
static int modem_pm_driver_free(struct inode *inode, struct file *file)
{
	...
	if (down_interruptible(&modem_pm_driver_lock) == 0) {
		...
		//resource_release("vdd2_opp", &my_device); //TODO

		/* Remove the setting of the latency constraint */
		//resource_release("core_latency", &my_device); //TODO
				 ...
				 unsigned long arg)
{
	...
	case MODEM_PM_DRIVER_IOCTL_HANDLE_FREQUENCY_OPP_CONSTRAINT: {
		...
		//	resource_request("vdd2_opp", &my_device, 600000); //TODO
			status = 0;
			break;
		}
	...
	case MODEM_PM_DRIVER_IOCTL_HANDLE_LOW_POWER_POLICY_CONSTRAINT: {
		...
		//	resource_request("core_latency", &my_device, 9999); //TODO
			status = 0;
			break;
		}
		case MODEM_PM_SHARED_DDR_LOW_POWER_POLICY_RET: {
			/* Set the target value of the latency constraint */
		//	resource_request("core_latency", &my_device, 39999); //TODO
			status = 0;
			break;
		}
