...
{
	Des3DesReg_t *pDESReg_t = (Des3DesReg_t *)IO_ADDRESS(
							DES2_REGS_HW_ADDR);
			...
			u8 *pSrc, u8 *pDest, u32 nbBlocks)
{
	 ...
	 */
	if (nbBlocks * DES_BLOCK_SIZE >= DMA_TRIGGER_POLLING_DES) {
					...
					u32 nbBlocks, u32 dmaUse)
{
	...
	while (nLength > 0) {
		...
		scxPublicDMAStart(
			dma_ch1, OMAP_DMA_DROP_IRQ | OMAP_DMA_BLOCK_IRQ);
		...
		if (dmaUse == PUBLIC_CRYPTO_DMA_USE_IRQ) {
			...
			dprintk(KERN_INFO
				"PDrvCryptoUpdateDESWithDMA: Polling DMA\n");
