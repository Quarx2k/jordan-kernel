...
static unsigned int panel_debug = 0;
#define DBG(format, ...) \
	if (panel_debug) \
...
#define MAPPHONE_ESD_CHECK_PERIOD   msecs_to_jiffies(16000)
...
static void mapphone_esd_work(struct work_struct *work)
{
	...
	if (!mp_data->enabled) {
		printk("%s: ignoring because display is disabled\n", __func__);
					...
					u16 x, u16 y, u16 w, u16 h)
{
	...
	if (dssdev->phy.dsi.type == OMAP_DSS_DSI_TYPE_CMD_MODE) {
		...
		r = mapphone_set_update_window(dssdev, x, y, w, h);
		if (r)
			goto err;
	}
...
static int dsi_mipi_cm_480_854_panel_enable(struct omap_dss_device *dssdev)
{
	...
	/* Check display model */ 
	/* Defy Green lense - AUO panel */
	/* Defy Red lense and Defy+ - TMD panel */
	if (dssdev->panel.panel_id == MOT_DISP_MIPI_CM_370_480_854) {
		if (read_supplier_id(dssdev) ==  CTL_SUPPLIER_ID_TMD) {
			...
			panel_data->te_scan_line = 0x80;
			printk(KERN_INFO "Overwrite te_scan_line=%d\n",
				panel_data->te_scan_line);
		} else if (read_supplier_id(dssdev) == CTL_SUPPLIER_ID_AUO) {
			DBG("dsi_mipi_cm_480_854_panel_enable() - AUO panel\n");
